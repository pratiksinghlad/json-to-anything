...
import { G as require_react, W as require_react_dom } from "./mui-vendor-CE9s5qa2.js";
...
var require_react_dom_client_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	var Scheduler = require_scheduler(), React$2 = require_react(), ReactDOM = require_react_dom();
	...
	function setValueForStyles(node, styles$1, prevStyles) {
		if (null != styles$1 && "object" !== typeof styles$1) throw Error(formatProdErrorMessage(62));
		node = node.style;
		if (null != prevStyles) {
			for (var styleName in prevStyles) !prevStyles.hasOwnProperty(styleName) || null != styles$1 && styles$1.hasOwnProperty(styleName) || (0 === styleName.indexOf("--") ? node.setProperty(styleName, "") : "float" === styleName ? node.cssFloat = "" : node[styleName] = "");
			for (var styleName$16 in styles$1) styleName = styles$1[styleName$16], styles$1.hasOwnProperty(styleName$16) && prevStyles[styleName$16] !== styleName && setValueForStyle(node, styleName$16, styleName);
		} else for (var styleName$17 in styles$1) styles$1.hasOwnProperty(styleName$17) && setValueForStyle(node, styleName$17, styles$1[styleName$17]);
	}
	...
	function shallowEqual(objA, objB) {
		...
		for (keysB = 0; keysB < keysA.length; keysB++) {
			...
			if (!hasOwnProperty.call(objB, currentKey) || !objectIs$1(objA[currentKey], objB[currentKey])) return !1;
		}
		return !0;
	}
	...
	function areHookInputsEqual(nextDeps, prevDeps) {
		...
		for (var i = 0; i < prevDeps.length && i < nextDeps.length; i++) if (!objectIs$1(nextDeps[i], prevDeps[i])) return !1;
		return !0;
	}
	...
	function updateReducerImpl(hook, current, reducer) {
		...
		if (null === baseQueue) hook.memoizedState = pendingQueue;
			...
			if (!objectIs$1(pendingQueue, hook.memoizedState) && (didReceiveUpdate = !0, didReadFromEntangledAsyncAction$60 && (reducer = currentEntangledActionThenable, null !== reducer))) throw reducer;
	...
	function updateDeferredValueImpl(hook, prevValue, value, initialValue) {
		...
		if (null !== currentTreeHiddenStackCursor.current) return hook = mountDeferredValueImpl(hook, value, initialValue), objectIs$1(hook, prevValue) || (didReceiveUpdate = !0), hook;
	...
	function dispatchSetStateInternal(fiber, queue, action, lane) {
		...
		if (isRenderPhaseUpdate(fiber)) enqueueRenderPhaseUpdate(queue, update);
			...
			if (0 === fiber.lanes && (null === alternate || 0 === alternate.lanes) && (alternate = queue.lastRenderedReducer, null !== alternate)) try {
				...
				if (objectIs$1(eagerState, currentState)) return enqueueUpdate$1(fiber, queue, update, 0), null === workInProgressRoot && finishQueueingConcurrentUpdates(), !1;
			} catch (error) {}
	...
	function isRenderConsistentWithExternalStores(finishedWork) {
		for (var node = finishedWork;;) {
			...
			if ((0 === tag || 11 === tag || 15 === tag) && node.flags & 16384 && (tag = node.updateQueue, null !== tag && (tag = tag.stores, null !== tag))) for (var i = 0; i < tag.length; i++) {
				...
				check = check.value;
				try {
					if (!objectIs$1(getSnapshot(), check)) return !1;
				} catch (error) {
					return !1;
				}
			}
	...
	function preinitStyle(href, precedence, options$1) {
		...
		var ownerDocument = globalDocument;
		if (ownerDocument && href) {
			var styles$1 = getResourcesFromRoot(ownerDocument).hoistableStyles, key = getStyleKey(href);
			precedence = precedence || "default";
			var resource = styles$1.get(key);
			if (!resource) {
				var state = {
					loading: 0,
					preload: null
	...
	ReactDOMHydrationRoot.prototype.render = ReactDOMRoot.prototype.render = function(children) {
		...
		var current = root$1.current, lane = requestUpdateLane();
		updateContainerImpl(current, lane, children, root$1, null, null);
	};
	...
	var isomorphicReactPackageVersion$jscomp$inline_1840 = React$2.version;
...
const hasLoadedNamespace = (ns, i18n, options$1 = {}) => {
	...
	return i18n.hasLoadedNamespace(ns, {
		...
		precheck: (i18nInstance$1, loadNotPending) => {
			if (options$1.bindI18n && options$1.bindI18n.indexOf("languageChanging") > -1 && i18nInstance$1.services.backendConnector.backend && i18nInstance$1.isLanguageChangingTo && !loadNotPending(i18nInstance$1.isLanguageChangingTo, ns)) return false;
		}
	});
};
...
var require_use_sync_external_store_shim_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var objectIs = "function" === typeof Object.is ? Object.is : is, useState$1 = React$1.useState, useEffect$1 = React$1.useEffect, useLayoutEffect = React$1.useLayoutEffect, useDebugValue = React$1.useDebugValue;
...
var import_shim = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function getHistoryState(location, index$1) {
	return {
		usr: location.state,
		key: location.key,
		idx: index$1
...
function getUrlBasedHistory(getLocation, createHref2, validateLocation, options$1 = {}) {
	...
	function handlePop() {
		...
		let delta = nextIndex == null ? null : nextIndex - index$1;
		index$1 = nextIndex;
	...
	function push$2(to, state) {
		action = "PUSH";
		let location = createLocation(history.location, to, state);
		if (validateLocation) validateLocation(location, to);
		index$1 = getIndex() + 1;
		let historyState = getHistoryState(location, index$1);
...
function flattenRoutes(routes, branches = [], parentsMeta = [], parentPath = "", _hasParentOptionalSegments = false) {
	...
	routes.forEach((route, index$1) => {
		if (route.path === "" || !route.path?.includes("?")) flattenRoute(route, index$1);
		else for (let exploded of explodeOptionalSegments(route.path)) flattenRoute(route, index$1, true, exploded);
	});
	return branches;
}
...
function matchPath(pattern, pathname) {
	...
	return {
		params: compiledParams.reduce((memo2, { paramName, isOptional }, index$1) => {
			if (paramName === "*") {
				let splatValue = captureGroups[index$1] || "";
				pathnameBase = matchedPathname.slice(0, matchedPathname.length - splatValue.length).replace(/(.)\/+$/, "$1");
			}
			const value = captureGroups[index$1];
...
import_react.createContext(false);
...
function useHref(to, { relative } = {}) {
	...
	let { basename, navigator: navigator$1 } = import_react.useContext(NavigationContext);
	let { hash, pathname, search } = useResolvedPath(to, { relative });
	let joinedPathname = pathname;
	if (basename !== "/") joinedPathname = pathname === "/" ? basename : joinPaths([basename, pathname]);
	return navigator$1.createHref({
...
function useRoutesImpl(routes, locationArg, dataRouterState, unstable_onError, future) {
	...
	let { navigator: navigator$1 } = import_react.useContext(NavigationContext);
...
function _renderMatches(matches, parentMatches = [], dataRouterState = null, unstable_onError = null, future = null) {
	...
	return renderedMatches.reduceRight((outlet, match, index$1) => {
		...
		let matches2 = parentMatches.concat(renderedMatches.slice(0, index$1 + 1));
		let getChildren = () => {
			let children;
...
function Router({ basename: basenameProp = "/", children = null, location: locationProp, navigationType = "POP", navigator: navigator$1, static: staticProp = false }) {
...
function createRoutesFromChildren(children, parentPath = []) {
	let routes = [];
	import_react.Children.forEach(children, (element, index$1) => {
		if (!import_react.isValidElement(element)) return;
		let treePath = [...parentPath, index$1];
	...
	if (isFormElement(target)) {
		...
		if (!isFormDataSubmitterSupported()) {
			...
			if (type === "image") {
				...
				formData.append(`${prefix$1}x`, "0");
				formData.append(`${prefix$1}y`, "0");
...
var NavLink = import_react.forwardRef(function NavLinkWithRef({ "aria-current": ariaCurrentProp = "page", caseSensitive = false, className: classNameProp = "", end = false, style: styleProp, to, viewTransition, children,...rest }, ref) {
	...
	let { navigator: navigator$1, basename } = import_react.useContext(NavigationContext);
	let isTransitioning = routerState != null && useViewTransitionState(path) && viewTransition === true;
	let toPathname = navigator$1.encodeLocation ? navigator$1.encodeLocation(path).pathname : path.pathname;
	...
	let className$1;
	if (typeof classNameProp === "function") className$1 = classNameProp(renderProps);
	else className$1 = [
		classNameProp,
...
function useSubmit() {
	...
	return import_react.useCallback(async (target, options$1 = {}) => {
		...
		if (options$1.navigate === false) {
			let key = options$1.fetcherKey || getUniqueFetcherId();
			await router.fetch(key, currentRouteId, options$1.action || action, {
		...
		} else await router.navigate(options$1.action || action, {
			preventScrollReset: options$1.preventScrollReset,
			formData,
			body,
			formMethod: options$1.method || method,
...
function useScrollRestoration({ getKey, storageKey } = {}) {
	...
	let location = useLocation();
	let matches = useMatches();
	let navigation$1 = useNavigation();
	import_react.useEffect(() => {
...
var require_lib = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var Editor = React.forwardRef(function Editor$1(props, ref) {
		var autoFocus = props.autoFocus, disabled = props.disabled, form = props.form, highlight = props.highlight, _a = props.ignoreTabKey, ignoreTabKey = _a === void 0 ? false : _a, _b = props.insertSpaces, insertSpaces = _b === void 0 ? true : _b, maxLength = props.maxLength, minLength = props.minLength, name = props.name, onBlur = props.onBlur, onClick = props.onClick, onFocus = props.onFocus, onKeyDown = props.onKeyDown, onKeyUp = props.onKeyUp, onValueChange = props.onValueChange, _c = props.padding, padding = _c === void 0 ? 0 : _c, placeholder = props.placeholder, preClassName = props.preClassName, readOnly = props.readOnly, required = props.required, style$1 = props.style, _d = props.tabSize, tabSize = _d === void 0 ? 2 : _d, textareaClassName = props.textareaClassName, textareaId = props.textareaId, value = props.value, rest = __rest(props, [
			"autoFocus",
			"disabled",
			"form",
			"highlight",
		...
		var recordChange = React.useCallback(function(record, overwrite) {
			...
			var _d$1 = historyRef.current, stack = _d$1.stack, offset = _d$1.offset;
			...
			if (overwrite) {
				...
				if (last && timestamp - last.timestamp < HISTORY_TIME_GAP) {
					...
					var previous = (_a$1 = getLines(last.value, last.selectionStart).pop()) === null || _a$1 === void 0 ? void 0 : _a$1.match(re);
					var current = (_b$1 = getLines(record.value, record.selectionStart).pop()) === null || _b$1 === void 0 ? void 0 : _b$1.match(re);
					if ((previous === null || previous === void 0 ? void 0 : previous[1]) && ((_c$1 = current === null || current === void 0 ? void 0 : current[1]) === null || _c$1 === void 0 ? void 0 : _c$1.startsWith(previous[1]))) {
		...
		var recordCurrentState = React.useCallback(function() {
			...
			if (!input) return;
			var value$1 = input.value, selectionStart = input.selectionStart, selectionEnd = input.selectionEnd;
			recordChange({
				value: value$1,
				selectionStart,
				selectionEnd
		...
		var updateInput$1 = function(record) {
			var input = inputRef.current;
			if (!input) return;
		...
		var applyEdits = function(record) {
			var input = inputRef.current;
			var last = historyRef.current.stack[historyRef.current.offset];
			if (last && input) historyRef.current.stack[historyRef.current.offset] = __assign(__assign({}, last), {
				selectionStart: input.selectionStart,
				selectionEnd: input.selectionEnd
			});
			recordChange(record);
			updateInput$1(record);
		};
		var undoEdit = function() {
			var _a$1 = historyRef.current, stack = _a$1.stack, offset = _a$1.offset;
		...
		var handleKeyDown = function(e) {
			...
			var _a$1 = e.currentTarget, value$1 = _a$1.value, selectionStart = _a$1.selectionStart, selectionEnd = _a$1.selectionEnd;
			var tabCharacter = (insertSpaces ? " " : "	").repeat(tabSize);
			if (e.key === "Tab" && !ignoreTabKey && capture) {
				e.preventDefault();
				if (e.shiftKey) {
					var linesBeforeCaret = getLines(value$1, selectionStart);
					var startLine_1 = linesBeforeCaret.length - 1;
					var endLine_1 = getLines(value$1, selectionEnd).length - 1;
					var nextValue = value$1.split("\n").map(function(line$1, i) {
						if (i >= startLine_1 && i <= endLine_1 && line$1.startsWith(tabCharacter)) return line$1.substring(tabCharacter.length);
						return line$1;
					}).join("\n");
					if (value$1 !== nextValue) {
						...
						applyEdits({
							...
							selectionEnd: selectionEnd - (value$1.length - nextValue.length)
						});
					}
				} else if (selectionStart !== selectionEnd) {
					var linesBeforeCaret = getLines(value$1, selectionStart);
					var startLine_2 = linesBeforeCaret.length - 1;
					var endLine_2 = getLines(value$1, selectionEnd).length - 1;
					var startLineText = linesBeforeCaret[startLine_2];
					applyEdits({
						value: value$1.split("\n").map(function(line$1, i) {
							if (i >= startLine_2 && i <= endLine_2) return tabCharacter + line$1;
							return line$1;
						}).join("\n"),
					...
					applyEdits({
						value: value$1.substring(0, selectionStart) + tabCharacter + value$1.substring(selectionEnd),
				...
				if (value$1.substring(0, selectionStart).endsWith(tabCharacter) && !hasSelection) {
					e.preventDefault();
					var updatedSelection = selectionStart - tabCharacter.length;
					applyEdits({
						value: value$1.substring(0, selectionStart - tabCharacter.length) + value$1.substring(selectionEnd),
						selectionStart: updatedSelection,
						selectionEnd: updatedSelection
					});
				}
				...
				if (selectionStart === selectionEnd) {
					var line = getLines(value$1, selectionStart).pop();
					...
					if (matches === null || matches === void 0 ? void 0 : matches[0]) {
						...
						applyEdits({
							value: value$1.substring(0, selectionStart) + indent + value$1.substring(selectionEnd),
							selectionStart: updatedSelection,
							selectionEnd: updatedSelection
						});
					}
				...
				else if (e.keyCode === KEYCODE_BRACKETS) if (e.shiftKey) chars = ["{", "}"];
				else chars = ["[", "]"];
				else if (e.keyCode === KEYCODE_QUOTE) if (e.shiftKey) chars = ["\"", "\""];
				else chars = ["'", "'"];
				else if (e.keyCode === KEYCODE_BACK_QUOTE && !e.shiftKey) chars = ["`", "`"];
				if (selectionStart !== selectionEnd && chars) {
					e.preventDefault();
					applyEdits({
						value: value$1.substring(0, selectionStart) + chars[0] + value$1.substring(selectionStart, selectionEnd) + chars[1] + value$1.substring(selectionEnd),
						selectionStart,
		...
		React.useImperativeHandle(ref, function() {
			return {
				get session() {
					return { history: historyRef.current };
				},
				set session(session) {
		...
		return React.createElement("div", __assign({}, rest, { style: __assign(__assign({}, styles.container), style$1) }), React.createElement("pre", __assign({
			className: preClassName,
			"aria-hidden": "true",
			style: __assign(__assign(__assign({}, styles.editor), styles.highlight), contentStyle)
		}, typeof highlighted === "string" ? { dangerouslySetInnerHTML: { __html: highlighted + "<br />" } } : { children: highlighted })), React.createElement("textarea", {
			ref: function(c) {
...
export { useLocation as a, useTranslation as c, Routes as i, initReactI18next as l, BrowserRouter as n, useNavigate as o, Route as r, __vitePreload as s, require_lib as t, require_client as u };
